...
class CheckoutController extends Controller
{
    ...
    public function index()
    {
        ...
        // Chỉ lấy quận/huyện theo tỉnh đã chọn và phường/xã theo quận đã chọn
        $selectedProvinceCode = $user->province_id ?? null;
        $selectedDistrictCode = $user->district_id ?? null;

        if ($selectedProvinceCode) {
            $districts = $this->districtRepository->findByProvinceCode($selectedProvinceCode);
        } else {
            $districts = collect();
        }

        if ($selectedDistrictCode) {
            $wards = $this->wardRepository->findByDistrictCode($selectedDistrictCode);
        } else {
            $wards = collect();
        }
        $config = $this->config();
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'phone' => [
                'required',
                'string',
                'regex:/^(0[3|5|7|8|9][0-9]{8}|01[2689][0-9]{7})$/',
                'min:10',
                'max:11'
            ],
