...
class ProductController extends Controller
{
    ...
    {
        $categories = $this->categoryRepository->getParentCategory()->whereNull('deleted_at');
        $brands = $this->categoryRepository->findByWhereIn('is_room', [1], ['products'], ['id', 'name', 'slug'])->whereNull('deleted_at');
        $query = Product::where('publish', 1)->whereNull('deleted_at');
        ...
        if ($request->filled('category_id')) {
            $category = $this->categoryRepository->findById($request->category_id, ['products']);
            if($category->is_room == 2 && $category->parent_id == null) {
