...
class ProductService extends BaseService
{
    ...
    public function getProductBySku($sku)
    {
        if (isset($sku)) {
            ...
            if (isset($data->quantity)) {
                // Nếu là variant, sử dụng số lượng của variant
                $data->quantity = $data->quantity;
            } elseif (isset($data->product) && isset($data->product->quantity)) {
                // Nếu không có variant, sử dụng số lượng của sản phẩm chính
    ...
    // Method để cập nhật số lượng tổng của sản phẩm dựa trên các variant
    public function updateProductTotalQuantity($productId)
    {
        $product = $this->productRepository->findById($productId);
        if ($product && $product->has_attribute == 1) {
