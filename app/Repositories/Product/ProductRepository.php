...
class ProductRepository extends BaseRepository
{
    ...
    // Giả sử trong ProductRepository
public function updateQuantity($productId, $minusQuantity)
{
    $product = Product::find($productId);

    if (!$product) return false;

    // ✅ CHẶN nếu số lượng hiện tại không đủ
    if ($product->quantity < $minusQuantity) {
        \Log::warning("Không đủ số lượng sản phẩm để trừ", [
    ...
    //             DB::raw('product_variants.code AS variant_code'),
...
public function getTopSellingProducts()
{
    $products = DB::table('products')
        ->leftJoin('product_variants', 'products.id', '=', 'product_variants.product_id')
        ->leftJoin('order_details', function ($join) {
            $join->on('order_details.sku', '=', 'products.sku')
                ->orOn('order_details.sku', '=', 'product_variants.sku');
        })
        ->join('orders', function ($join) {
