                    ...
                    @foreach ($posts as $item)
                        <div class="swiper-slide">
                            <div class="card-grid-style-1">
                                <div class="image-box">
                                    <a href="{{ url('/tin-tuc/' . (preg_match('/\.html$/i', $item->slug ?? '') ? preg_replace('/\.html$/i', '', $item->slug) : $item->slug)) }}">
                                    </a>
                                    <img src="{{ $item->thumbnail }}" alt="Ecom"></div>
                                <a class="color-gray-1100"
                                    href="{{ url('/tin-tuc/' . (preg_match('/\.html$/i', $item->slug ?? '') ? preg_replace('/\.html$/i', '', $item->slug) : $item->slug)) }}">
                                    <h4>{{ $item->title }}</h4>
                                </a>
