...
@section('content')
    ...
    <div class="section-box shop-template mt-30 mb-30">
        <div class="container">
            @if ($keyword)
                <div class="row mb-30">
                    <div class="col-12">
                                        ...
                                        <div class="card-grid-inner">
                                            <div class="tools">
                                                <a class="btn btn-wishlist btn-tooltip mb-10" href="#"
                                                    product-id="{{ $product->id }}" aria-label="Yêu thích"></a>
                                            </div>
                                            <div class="image-box">
                                            ...
                                            <div class="info-right">
                                                ...
                                                <div class="price-info">
                                                    ...
                                                    @if ($product->discount > 0)
                                                        <span
                                                            class="color-gray-500 price-line">{{ formatMoney($product->price) }}</span>
                                                    @endif
                                                </div>
                                                <div class="mt-20 box-btn-cart"><a class="btn btn-cart"
                                                        href="{{ route('client.product.detail', $product->slug) }}">Xem chi
                                                        tiết</a></div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        @else
                            <div class="col-12">
                                <div class="text-center py-5">
                                    ...
                                    <p class="text-muted">Hãy thử tìm kiếm với từ khóa khác hoặc <a
                                            href="{{ route('client.home') }}">về trang chủ</a></p>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                <div class="col-lg-3 order-last order-lg-first">
                    ...
                    <div class="sidebar-border mb-20">
                        ...
                        <div class="sidebar-content">
                            <form method="GET" action="{{ route('client.product.index') }}">
                                ...
                                <a href="{{ route('client.product.index') }}" class="btn btn-brand-3 w-100 mt-2">Đặt lại
                                    tìm kiếm</a>
                            </form>
                        </div>
                    </div>
                    ...
                    @if (!$brand_id)
                        <div class="sidebar-border mb-0">
                            ...
                            <div class="sidebar-content">
                                <ul class="list-nav-arrow category-list">
                                    @foreach ($categories as $category)
                                        <li class="category-item @if ($category->id == $category_id) cate-active @endif">
                                            <a
                                                href="{{ route('client.product.index', array_filter(array_merge(request()->all(), ['category_id' => $category->id, 'brand_id' => $brand_id]))) }}">
                                                {{ $category->name }}
                                            </a>
                                            ...
                                            @if ($category->children->count() > 0)
                                                <ul class="subcategory-list">
                                                    @foreach ($category->children as $child)
                                                        <li
                                                            class="subcategory-item @if ($child->id == $category_id) cate-active @endif">
                                                            <a
                                                                href="{{ route('client.product.index', array_filter(array_merge(request()->all(), ['category_id' => $child->id, 'brand_id' => $brand_id]))) }}">
                                                                {{ $child->name }}
                                                            </a>
                                                        </li>
                                                    @endforeach
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            categoryItems.forEach(item => {
                ...
                if (subList) {
                    if (!item.classList.contains("cate-active") && !subList.querySelector(".cate-active")) {
