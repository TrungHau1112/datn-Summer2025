        ...
        <div class="actions">
            @if (isset($createButton['method']) && $createButton['method'])
                <form action="{{ route($createButton['route']) }}" method="post">
                    ...
                    <button class="btn btn-primary d-inline-flex justify-content-center" type="submit">
                        <i data-feather="refresh-cw" class="me-1"></i>
                        {{ $createButton['label'] ?? '' }}
                    </button>
                </form>
            @else
                <a href="{{ route($createButton['route']) }}" class="btn btn-primary d-inline-flex justify-content-center">
