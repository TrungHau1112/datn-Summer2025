    ...
    <x-form :config="$config" :model="$user ?? null">
        <div class="row">
            <div class="col-xl-9">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-lg-12">
                                <x-input :label="'Số điện thoại'" :name="'phone'" :value="$user->phone ?? ''"
                                    :required="true" />
                            </div>
                        </div>
                        ...
                        <div class="col-lg-12">
                            <x-input :label="'Địa chỉ cụ thể'" :name="'address'" :value="$user->address ?? ''"
                                :required="false" />
                        </div>
                    </div>
                </div>
                                    ...
                                    <tbody>
                                        @if (isset($user->orders) && count($user->orders))
                                            @foreach ($user->orders as $key => $order)
                                                <tr>
                                                    <th>{{ $key + 1 }}</th>
                                                    <td><a href="{{ route('order.edit', $order->id) }}">{{ $order->code }}</a>
                                                    </td>
            ...
            <div class="col-xl-3">
                ...
                <x-thumbnail :label="'Ảnh đại diện'" :name="'avatar'" :value="$user->avatar ?? 'https://placehold.co/600x600?text=Thoi%20Trang%20Quan%20Ao'" />
